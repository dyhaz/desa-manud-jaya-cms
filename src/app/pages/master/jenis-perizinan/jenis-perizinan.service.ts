...
import { Table, SearchResult } from './jenis-perizinan.model';
...
export class JenisIzinTableService {
  ...
  private async _search(): Promise<Observable<SearchResult>> {
    ...
    try {
      let result;
      if (!this.tableData) {
        result = await this.jenisPerizinanService.getAllJenisPerizinan().toPromise();
